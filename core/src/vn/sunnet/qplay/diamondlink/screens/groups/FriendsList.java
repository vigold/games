...
public class FriendsList extends Group {
	...
	TextureRegion noAvatar = null;
	
	Random random = new Random();
	private BitmapFont provinceFont;
	private Label leftDays;
	
	public FriendsList(DiamondLink game) {
...
//		provinceFont = generator.generateFont(14, AbstractScreen.FONT_CHARACTERS, false);
//		provinceFont.getRegion().getTexture().setFilter(TextureFilter.Linear, TextureFilter.Linear);
	...
	private void initContent() {
		...
		Image im = new Image(atlas.findRegion("FriendRankFrame"));
		im.setBounds(0, 0, 400, 464);
		addActor(im);
		setBounds(0, 0, 400, 464);
		
		friendContent = new Table();
		friendScroll = new ScrollPane(friendContent);
		friendScroll.setBounds(11, 13 + plusY, 377, 356);
		friendScroll.setScrollingDisabled(true, false);
		friendScroll.setScrollY(100);
		addActor(friendScroll);
...
//	public void updateContent(final EsObject myRank, final EsObject[] ranks) {
//		final float addPixel = 40;
//		final float lineWidth = 374;
//		final float lineHeight = 85;
//		
//		friendContent.clearChildren();
//		
//		TextureAtlas atlas = manager.get(UIAssets.MENU_FG, TextureAtlas.class);
...
//		group.addActor(rank);
//		
//		String avatar = ""+myRank.getInteger(Fields.AVATAR);
//		TextureRegion avatarRegion = getDisplay(avatar);
...
//			modeIcon.setDrawable(new TextureRegionDrawable(
//					getModeOfScore(myRank.getInteger(Fields.MODE_OF_SCORE))));
//		
...
//		group.addActor(label);
//		label = new Label(""
//				+ Operator.convertNumberToString((int) myRank
//						.getLong(Fields.BESTSCORE)), new LabelStyle(scoreFont,
//				scoreFont.getColor()));
//		label.setWrap(true);
...
//		label.setWrap(true);
//		label.setAlignment(Align.left|Align.bottom);
//		label.setBounds(lineWidth - 50, 57, 50, 69);
//		group.addActor(label);
//		
...
//		group.addActor(label);
//		
//		friendContent.add(group).pad(1, 1, 1, 1).minWidth(lineWidth).minHeight(lineHeight).fill();
//		friendContent.row();
//		
//		// listrank
...
//			}
//			
//			avatar = ""+ranks[i].getInteger(Fields.AVATAR);
//			avatarRegion = getDisplay(avatar);
//			if (avatarRegion == null )
//				avatarRegion = atlas.findRegion("NoAvatar");
//			
...
//				modeIcon.setDrawable(new TextureRegionDrawable(
//						getModeOfScore(ranks[i].getInteger(Fields.MODE_OF_SCORE)))); 
//			
...
//			
//			label = new Label(""
//					+ Operator.convertNumberToString((int) ranks[i]
//							.getLong(Fields.BESTSCORE)), new LabelStyle(
...
//		System.out.println("FriendList update");
//	}
	...
	public void updateAvatars(Avatar[] avatars, int maxAvatar) {
		...
		if (display != null) {
			...
			if (myAvatarInRanks != null)
				myAvatarInRanks
						.setDrawable(new TextureRegionDrawable(noAvatar));
		}
	}
	...
	public boolean isEmpty() {
		return friendContent.hasChildren();
	}
