...
public class ClassicSummary extends ScoreSummary {
	
	private Label timeLabel;
	private Label comboLabel;
	private Label comboMax;

	public ClassicSummary(DiamondLink game, GameScreen screen,
	...
	protected void initContent() {
		...
		im = new Image(atlas.findRegion("Time"));
		im.setBounds(134 - im.getPrefWidth() / 2, 192 + 83 + 83 / 2 - im.getPrefHeight() / 2, im.getPrefWidth(), im.getPrefHeight());
		panel.addActor(im);
		
		im = new Image(atlas.findRegion("Combo"));
		im.setBounds(134 - im.getPrefWidth() / 2, 192 + 83 / 2 - im.getPrefHeight() / 2, im.getPrefWidth(), im.getPrefHeight());
		panel.addActor(im);
		...
		timeLabel = new Label("0", new Label.LabelStyle(font, font.getColor()));
		timeLabel.setBounds(213, 192 + 83 * 1 + 83 / 2 - timeLabel.getPrefHeight() / 2, 209, 51);
		timeLabel.setWrap(true);
		timeLabel.setAlignment(Align.center, Align.center);
		panel.addActor(timeLabel);
	...
	public void updateContent(float addExp, int plusExpPercent, float addCoins,
			int plusCoinPercent, float newScore, float bestScore, int time, int combo, int max) {
