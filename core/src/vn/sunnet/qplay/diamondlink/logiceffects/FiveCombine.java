...
public class FiveCombine extends Effect {
	
	float limitTime = 0;
	
	public FiveCombine(GameLogic logic,GameScreen screen) {
	...
	public void parse(IFunctions iFunctions, String prefix) {
		...
		for (int i = 0; i < realTarget.size(); i++) {
			...
			String data = iFunctions.getFastString(prefix+" fivecombine "+i, "");
			if (data != "") {
	...
	public void save(IFunctions iFunctions, String prefix) {
		...
		for (int i = 0; i < realTarget.size(); i++) {
			...
			iFunctions.putFastString(prefix+" fivecombine "+i, diamond.getDiamondValue()+"|"+sourX+"|"+sourY+"|"+x+"|"+y+"|"+desX+"|"+desY+"|"+time);
		}
	}
...
//		}
	...
	public void init() {
		...
		Sound sound = screen.manager.get(SoundAssets.FIVE_COMBINE_SOUND, Sound.class);
		...
		for (int i = 0 ; i < realTarget.size() ; i++) {
			...
			dDiamond.setAction(Diamond.COMBINE_MOVE);
				...
				dDiamond.setAction(Diamond.COMBINE_MOVE);
				dDiamond.setFinish(Diamond.COMBINE_MOVE);
			}
...
//		Assets.playSound(Assets.FiveCombine, 0);
		int cell = source[0];
		IDiamond dDiamond = screen.diamonds.get(cell);
		int row = cell / 8; 
		int col = cell % 8;
		...
		logic.grid[row][col] = logic.getDiamondValue(Diamond.FIVE_COLOR_DIAMOND, dColor, 0);
