...
public class RCCombine extends Effect {
	
	
	
	private float limitTime;

	public RCCombine(GameLogic logic,GameScreen screen) {
		// TODO Auto-generated constructor stub
		super(logic, screen);
		type = ROW_COL_COMBINE;
		this.screen = screen;
		this.logic = logic;
		Priority = 3;
	}
	...
	public void parse(IFunctions iFunctions, String prefix) {
		...
		for (int i = 0; i < realTarget.size(); i++) {
			...
			String data = iFunctions.getFastString(prefix+" rccombine "+i, "");
			if (data != "") {
	...
	public void save(IFunctions iFunctions, String prefix) {
		...
		for (int i = 0; i < realTarget.size(); i++) {
			...
			iFunctions.putFastString(prefix+" rccombine "+i, diamond.getDiamondValue()+"|"+sourX+"|"+sourY+"|"+x+"|"+y+"|"+desX+"|"+desY+"|"+time);
		}
	}
	...
	public void concurrentResolve() {
		...
		int size = Math.min(maxRow - minRow, maxCol - minCol);
		int beginRow = minRow;
		int beginCol = minCol;
		while (beginRow + size < i) beginRow++;
		while (beginCol + size < j) beginCol++;
		for (int t = beginRow ; t < beginRow + size + 1 ; t++) {
			
			logic.effectOf[t][j].setEffect(this, false);
			//effect.addM_Targer(new Integer(t * 8 + j));
		}
		
		for (int t = beginCol ; t < beginCol + size + 1; t++) {
			
			logic.effectOf[i][t].setEffect(this, false);
			//effect.addM_Targer(new Integer(i * 8 + t));
		}
		
		for (int t = beginRow ; t < maxRow + 1 ; t++) {
			this.addM_Targer(new Integer(t * 8 + j));
		}
		
		for (int t = beginCol ; t < maxCol + 1; t++) {
			this.addM_Targer(new Integer(i * 8 + t));
		}
		logic.repairCombine(maxRow, minRow, maxCol, minCol, this);
	}
...
//		}
	...
	public void init() {
		...
		Sound sound = screen.manager.get(SoundAssets.T_COMBINE_SOUND, Sound.class);
		...
		for (int i = 0 ; i < realTarget.size() ; i++) {
			...
			else dDiamond.setAction(Diamond.COMBINE_MOVE);
			...
			//eatCell(row, col);
			if (logic.diamondType(logic.grid[row][col]) != IDiamond.NORMAL_DIAMOND) 
...
//			logic.gridFlag[row][col] = Operator.offBit(Effect.FIXED_TO_FALL, logic.gridFlag[row][col]);
			logic.gridFlag[row][col] = Operator.onBit(Effect.ROW_COL_COMBINE, logic.gridFlag[row][col]);
	...
	public void freeEffect() {
		...
		logic.grid[row][col] =  logic.getDiamondValue(Diamond.BLINK_DIAMOND, dColor, 0);
...
//		}
		...
		for (int i = 0; i < mirrorTarget.size(); i++) {
			...
			logic.gridFlag[row][col] = Operator.offBit(Effect.ROW_COL_COMBINE, logic.gridFlag[row][col]);
			if (logic.grid[row][col] == -1) { 
				logic.gridFlag[row][col] = Operator.offBit(Effect.FIXED_POS, logic.gridFlag[row][col]);
//				logic.gridFlag[row][col] = Operator.offBit(Effect.FIXED_TO_FALL, logic.gridFlag[row][col]);
			}
